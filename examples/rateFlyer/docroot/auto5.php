<?php
    ...
    $lead->maxDataValue('page', 5);
    $lead->saveLocal();
    if(isset($_POST['_is_submit'])) {
        //header('Location: /auto4');
        //we want to do a variant test here, so we ask what we should go to
        //@TODO: make this work
        $married_flow = $lead->retrievePrivateData('married_flow', 'some_default_flow_name');
        $flow_row = $lead->selectFlowRow($married_flow);
        ...
        <div class="container">
            <form name="mainForm" method="POST" class="form-horizontal">
                ...
                <div class="form-group">
                    <div class="col-xs-12">
                        <div class="hc3">Good Driver?</div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-xs-12">
                        <input type="submit" name="good_driver" value="YES" class="btn-lg btn-block btn-custom" />
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-xs-12">
                        <input type="submit" name="good_driver" value="NO" class="btn-lg btn-block btn-custom" />
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-xs-12">
                        <div class="hc4">Incident-free for the last 3 years</div>
                    </div>
                </div>
            </form>
        </div>
